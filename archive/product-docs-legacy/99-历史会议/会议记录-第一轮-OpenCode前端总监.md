...
产品证据：`D:\Magic\docs\01-产品\\90-支撑材料\\Magic产品系统边界与生命周期底图.md` 设计了工程、PM、项目成员、会话、任务、子任务、回合、运行实例、共享事实、产物和权限的分层。

产品证据：`D:\Magic\docs\01-产品\\00-项目管理\\项目总控.md:180-236` 建议由 Magic 任务账本保存产品任务的权威状态，OpenCode 运行事件、工具结果和 AgentCore 机制只作为运行证据。
...
证据要求：每项能力只能标记为“原生满足、适配后满足、Magic 需补建、当前不支持、仍未知”，并附版本、复现、限制、负责人和验收标准。这一要求来自 `D:\Magic\docs\01-产品\\90-支撑材料\\技术会议议题与裁定表.md:31-48`。
