// 搬自 plugins/magic-ceo-ui/src/client/selection.ts（2026-09-18 CEO 委派图卡接入）。
// 改动：import 改指本地纯函数 ../team.ts（原插件路径 ../team.ts 同源）。
// 说明：这是模块级单例（无 cordis 依赖），供图卡订阅「选中成员 / 团队名册」。
// 成员详情面板（CeoMemberInspector）本轮未搬，选中态目前只驱动画布高亮。
