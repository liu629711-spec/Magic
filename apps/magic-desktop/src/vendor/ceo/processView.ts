// 搬自 plugins/magic-ceo-ui/src/processView.ts（2026-09-18 CEO 委派图卡右坞成员详情接入）。
// 原样保留：无外部依赖的纯函数（工具名/参数/结果解析、搜索结果与抓页归一化）。
// 说明：CeoMemberInspector 的 produce 检测（../produced-files.ts）与
// CeoProcessTimeline 的过程明细都依赖本文件；图卡自身的 toolDisplayName 另存于 ../tool-display.ts。
