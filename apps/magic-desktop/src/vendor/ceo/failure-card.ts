// 搬自 plugins/magic-ceo-ui/src/failure-card.ts（2026-09-18 CEO 委派图卡接入）。
// 原样保留（纯数据层，无 React / 无 cordis）：失败/阻塞状态归一 + 阻塞成员等待对象。
// 本轮图卡只消费 waitingOnOf；failureCardOf 保留供后续失败卡/重试面复用。
