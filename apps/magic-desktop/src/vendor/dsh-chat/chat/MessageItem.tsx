// vendored from @deepseek-ai/dsh-client-ui-chat@0.1.5-rc.2 client/chat/MessageItem.tsx
// （整体为纯 props 渲染；ModelRetryItem 的倒计时与 PendingSubmissionBubble 的
// useMemo 属于组件内 React hooks，无任何 cordis/框架依赖，原样搬入。）

import { Fragment, memo, useEffect, useMemo, useState } from 'react'
import type { ReactNode } from 'react'
import type { FileAttachmentRef, ImageAttachmentRef, MessageImageSource } from '../vendor-types.ts'
import type { PendingSubmission } from '../vendor-types.ts'
...
import type { ModelRetryNode, TurnErrorNode } from '../vendor-types.ts'
import type { ReferencedUserMessageNode } from '../contract/chat-nodes.ts'
import { CompactionItem } from './CompactionItem.tsx'
import { ContextInjectionRow } from './ContextInjectionRow.tsx'
import { MessageIconActions } from './MessageIconActions.tsx'
import css from './MessageItem.module.css'

type UserImage = { type: 'image'; attachment: ImageAttachmentRef }
type UserFile = { type: 'file'; attachment: FileAttachmentRef }
...
function UserStyleBubble({
  ...
  /** 引用跳转回调（0.1.5-rc.2 primitives 的 projectUserText 不消费；保留类型面）。 */
    ...
    >
      <div className={css.userStack}>
        ...
        {showBubble && <div className={css.bubble}>
          {/* 安装的 @deepseek-ai/dsh-client-ui-primitives@0.1.5-rc.2 的
              projectUserText 为 4 参（无 references 落地参数），@提及装饰保留、
              引用跳转 affordance 不渲染；references prop 保留在类型面。 */}
...
/** 渲染一条 Host 权威的待准入插话，与其最终持久转录节点同一视觉语言。 */
...
/** 渲染一条本地提交回声，与其 Host 侧替代者同一视觉语言与 surface 标记。 */
...
/** User 与已准入 steering 的键控 Chat 渲染器。 */
export const UserMessageNodeView = memo(function UserMessageNodeView({
  node, renderMessageImages, openFile, openSkill, t,
}: ChatNodeViewProps<'user' | 'steering'>) {
  const data = node.data as ReferencedUserMessageNode
  return (
    <UserStyleBubble
