// vendored from @deepseek-ai/dsh-client-ui-conversation@<client> client/locales.ts
// （conversation 命名空间词典，ToolRow / ToolCallTree 的 t 座位使用；
// 原文件还承载 composer/queue 等文案，此处整字典照搬以保键集完整。）

import type { Translate } from '../vendor-types.ts'
import type { CommonKey } from './common.ts'

/** conversation 命名空间（Tool 渲染层由 conversation owner 供应）。 */
...
export const en = {
  ...
  'image.tooManyPixels': 'Image resolution is too large; compress it and try again',
  'image.dimensionTooLarge': 'Image sides must be at most {size}px; downscale and retry',
...
/** Tool 渲染层的 t 座位类型（收敛自 TranslateNS<'conversation'>）。 */
export type ConversationTranslate = Translate<ConversationKey | CommonKey>
