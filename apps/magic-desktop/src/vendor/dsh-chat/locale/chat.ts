// vendored from @deepseek-ai/dsh-client-ui-chat@0.1.5-rc.2 client/locale.ts

import type { Translate } from '../vendor-types.ts'
import type { CommonKey } from './common.ts'

/** Chat 命名空间（Chat target、节点、统计与详情文案）。 */
...
/**
 * Chat 渲染组件的 t 座位类型：chat 命名空间键 + common 基础词汇
 * （收敛自 ui-slots 的 PropsLocale<'chat'>['t']，即 TranslateNS<'chat'>）。
 */
export type ChatTranslate = Translate<ChatKey | CommonKey>

/**
 * 由词典构造 t 座位：壳层把选中的 zh/en 字典交给这里即可（chat 或
 * conversation 命名空间都适用）。查找顺序：命名空间 → common 基础词汇；
 * `{name}` 模板参数按序替换。行为等价替代 locale 服务的 bind(NS)。
 */
export function createTranslate<NSKey extends string>(
  ns: Record<NSKey, string>,
  common: Record<CommonKey, string>,
): Translate<NSKey | CommonKey> {
  return (key, params) => {
    const template = (ns as Record<string, string>)[key] ?? (common as Record<string, string>)[key] ?? key
...
/** chat 命名空间的便捷构造。 */
export function createChatTranslate(
  chat: Record<ChatKey, string>,
  common: Record<CommonKey, string>,
): ChatTranslate {
  return createTranslate<ChatKey>(chat, common)
}

/** conversation 命名空间的便捷构造。 */
export function createConversationTranslate(
  conversation: Record<import('./conversation.ts').ConversationKey, string>,
  common: Record<CommonKey, string>,
): import('./conversation.ts').ConversationTranslate {
  return createTranslate<import('./conversation.ts').ConversationKey>(conversation, common)
}
