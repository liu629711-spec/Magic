// vendored from @deepseek-ai/dsh-client-ui-chat@0.1.5-rc.2 client/conversation-nodes/command.ts
// （剥离：cordis Context 注册函数与 declare module 合并；
// isReplacementSurfaceEvent 来自 vendor-types 本地等价实现；
// CompactionCheckpointSource / CommandId 由 vendor-types 收敛。）

import type {
  CommandNode, CompactionCheckpointSource, CompactionSummaryNode, ConversationMatch,
  ConversationNodeContext, ConversationNodeDefinition,
} from '../vendor-types.ts'
import { isReplacementSurfaceEvent } from '../vendor-types.ts'
import type { ManualCompactionChatData } from '../contract/chat-nodes.ts'
import { chatNode } from './common.ts'

type CommandId = import('../vendor-types.ts').CommandId
...
/** 从压缩替换检查点读取关联身份。 */
export function compactSource(event: Parameters<ConversationNodeDefinition['match']>[0]): {
  compactionId: string
...
/** 由可选生命周期证据构建可见的摘要标记。 */
export function compactSummary(match: ConversationMatch | undefined, checkpoint: ConversationMatch): CompactionSummaryNode {
...
/** 把共享压缩证据折叠进 Definition 拥有的 State。 */
...
/** slash 指令生命周期（含集成的手动压缩）Definition。 */
