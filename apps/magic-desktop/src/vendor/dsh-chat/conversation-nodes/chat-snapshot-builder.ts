// vendored from @deepseek-ai/dsh-client-ui-chat@0.1.5-rc.2 client/conversation-nodes/chat-snapshot-builder.ts
// （剥离：
//  - @deepseek-ai/dsh-client-store 的 notifySubscribers → vendor-types 本地等价实现；
//  - ConversationViewBuilder/ConversationViewDefinition 实现契约由 vendor-types 收敛；
//  - cordis 注册函数 registerChatConversationView 与 chatViewDefinition 的注册壳移除，
//    ChatSnapshotBuilder 类与全部投影器原样保留（增量 replace/apply 语义不变）。）

import { notifySubscribers } from '../vendor-types.ts'
import type {
...
/**
 * 已加载轮的轮次栏投影：结构变化重推整组，内容 upsert 只重推受影响的轮，
 * 数组身份在条目未变时保持不变。
 */
...
/**
 * 在过程资格变化时保持既有相对顺序的前提下排序可见 Chat Node。
 * @param nodes - 当前物化的 Chat Node。
 * @returns 呈现顺序中的可见 Node。
 */
...
/** 把直接消息与紧随其后的 sourced recall 事件关联。 */
...
/** 以 `chat` 目标注册的增量 keyed Chat builder。 */
