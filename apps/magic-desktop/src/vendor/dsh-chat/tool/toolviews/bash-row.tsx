// vendored from @deepseek-ai/dsh-client-ui-tool@0.1.5-rc.2 client/tool/toolviews/bash-sample.tsx
// （剥离 useSessions → cwd prop，改名 bash-row；注册壳 bashToolviewSample 整体移除。）
...
export function BashRow({ toolName, block, cwd, inspect, t }: BashRowProps) {
  ...
  // 剥离点：源经 useSessions(list => list.byId[sessionId]?.cwd) 读取会话工作区；
  // 此处收敛为 ToolCallOwnerProps.cwd 纯 prop，缺省 undefined 时
  // terminalCardModel 行为不变（sessionCwd 参数本就可选）。
