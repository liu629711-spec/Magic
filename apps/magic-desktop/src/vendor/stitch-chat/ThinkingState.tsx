// vendored from stitch 设计稿画廊 ThinkingState（对话区 v2 皮肤，reasoning 节点换肤）。
// 嵌入改造：新增受控 running prop——受控模式跳过画廊自演时序（useSequence/STAGES），
// 运行中显示 shimmer 标题 + 全部行，结束显示 done 定稿；画廊模式（running 未传）行为
// 与设计稿一致。minHeight 稳定器仅画廊模式生效（对话流内折叠行保持紧凑）。
...
export default function ThinkingState({
  ...
  variant?: string;
  /** 受控运行态：传入后跳过画廊自演时序（undefined = 画廊演示模式） */
  running?: boolean;
  onSettled?: () => void;
  ...
  /* 受控模式：running 直驱；画廊模式：stage 直驱 */
  const controlled = running !== undefined;
  const working = controlled ? running : stage < 3;
  const autoExpanded = controlled ? running : stage >= 1 && stage < 4;
