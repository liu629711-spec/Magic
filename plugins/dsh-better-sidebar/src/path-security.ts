...
export async function ensureWorkspacePath(cwd: string, target: string, fence = true): Promise<string> {
  // Magic patch (2026-09-13, vendored copy): the official deliverable cards
  // open workspace artifacts with cwd-RELATIVE addresses (e.g.
  // "magic-browser/shot-x.png"), while upstream demands absolute targets and
  // rejects every relative one. Join relatives onto the session cwd first —
  // the realpath + containment fence below still guards traversal, so a
  // relative path is exactly as safe as an absolute one here.
  const joined = isAbsolute(target) ? target : join(cwd, target)
  const absolute = requireAbsolute(resolveSessionPath(cwd, joined))
