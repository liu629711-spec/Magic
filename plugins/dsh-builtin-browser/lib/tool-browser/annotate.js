 ...
 */
export function registerAnnotateTools(ctx, state, helpers) {
    const listSchema = {
        type: 'object',
    ...
    ctx.tools.register(defineTool({
        ...
        async execute(_args, exec) {
            ...
            if (!result.ok)
                throw new Error(`browser_annotate_enable: ${result.exception}`);
    ...
    ctx.tools.register(defineTool({
        ...
        output: {
            ...
            render: (_args, value) => {
                if (value.present !== true)
                    return [{ type: 'text', text: 'Annotation layer is not enabled on this page — call browser_annotate_enable first.' }];
                const items = value.items;
                if (items.length === 0)
                    return [{ type: 'text', text: 'No annotations on this page yet.' }];
                const lines = items.map((a, i) => `[${i + 1}] <${a.tag}> ${a.selector} — "${a.text}" (${a.rect.w}x${a.rect.h}, ${a.color}, ${a.font})${a.comment ? ` — 💬 ${a.comment}` : ''}`);
        ...
        async execute(args, exec) {
            ...
            if (!result.ok)
                throw new Error(`browser_annotations: ${result.exception}`);
            let parsed;
            try {
            ...
            return { present: true, items: (parsed.items ?? []) };
        },
    }));
