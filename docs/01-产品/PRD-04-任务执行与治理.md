...
> **【V1 范围注记】** 本节状态图为产品完整模型。第一版（V1）以 domain 源码的 Task 8 态为准：`proposed / ready / in_progress / awaiting_review / completed / blocked / cancelled / failed`；上图的"等待、部分完成、已停止、已归档"四个状态在 V1 中延后，不在界面或接口中伪造（见《前端契约确认清单》冲突 C2 与冻结决议 FZ-8）。
...
| MagicSession | 一段连续工作上下文 | 用户/系统；归档 | Magic 会话记录 |
...
用户、PM 或 Agent 提出目标时，系统创建任务草稿，记录原始输入、资源范围、工作方式、来源和临时责任人。草稿可以补充和修改，但无人负责时不能开始执行。
...
> **【V1 范围注记】** V1 的 Attempt 运行状态以 domain 源码 8 态为准：`created / admitted / running / cancelling / cancelled / succeeded / failed / unknown_after_restart`；本图的"排队、等待审批、暂停、已中断"在 V1 中延后——审批等待以"审批卡+横幅"呈现（不改状态机），同步缺口由"状态未知/同步缺口"表达（见《前端契约确认清单》冲突 C10、决议 P-1/FZ-1）。
...
|---|---|---|
| OpenCode 显示运行结束 | 一次底层执行停止了 | 不能直接显示任务成功 |
