...
> **2026-09-13 用户裁定**：任务板是**只读视图**。任务生命周期完全由智能体驱动——CEO 在主会话沟通后拆解派发（createTask），成员完成工作后带 CAS 与证据回写（updateTask）；前端不提供人工新建或完成任务的入口，人的角色是沟通与拍板，不是手工维护任务清单。成员已完成而任务仍待处理这类状态漂移按「状态诚实」原则原样透出，交由 CEO 处理，不得用人工点击抹平。
