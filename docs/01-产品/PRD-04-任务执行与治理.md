...
> **本版要点**：删除已作废的 domain 源码引用，运行状态改以插件实现为准（§3、§16）；F-10 改名并重定义为"底座契约核查"（§6、§13、§21）；阶段表对齐 PRD-01（§8）；工程组织已移除（§9）。

## 1. 目标

建立 Magic 自有的任务账本和交付闭环，把底座运行事实、产品任务状态、责任、权限、成本和外部副作用分开管理。交付契约与验收账本按 AgentCore 的产品能力迁移重建（见 [技术文档](../02-实现/01-DSH契约面与AgentCore能力映射.md) §4.2），不以成员自述文本判定完成。
...
> **【V1 范围注记】** 本节状态图为产品完整模型。V1 以 Magic 插件当前实现为准：交付判定见 `plugins/magic-ceo/src/delivery.ts:3-4` 的 `DeliveryStatus`（`completed / blocked / failed / partial / unverified`）；上图的"部分完成、已停止、已归档"等状态在 V1 中延后，不在界面或接口中伪造。

## 4. 权限、成本与副作用

权限、预算、风险和有效期分别治理。用户入口可以提供：请求批准、当前范围内自动处理、完全访问当前工作区三档选择，但三档入口不能突破资源边界、预算上限、高风险规则和系统禁止项。
...
成员的最终结构化输出必须由派发运行直接记录为父会话的可回放结果事件；成员间消息只用于运行中的补充通知，不能是交付回传的唯一通道。消息通道不可用、成员未给出结构化结果、或运行结束与结构化状态矛盾时，父任务显示“回传异常/待核实”，不得把底座运行结束视为成员完成。

调研、市场、竞品和多来源比较类结果必须同时给出可追溯来源与验证边界。若关键搜索或访问不可用、来源不足以支撑核心数字、或结果仅依赖未经核验的模型知识，成员状态必须是“阻塞”而不是“完成”或伪装成可交付的“部分完成”。是否接受证据降级由 CEO 基于用户目标提出明确方案，不得把技术质量裁定直接转交用户。

## 6. 开发准入：底座契约核查（原 F-10）

首次正式开发前必须锁定 DSH 版本、Profile/Bundle、插件加载方式与扩展点清单，并证明 Magic `TaskAttempt` 与底座执行证据的最小绑定模型，能在断连、事件缺口、进程重启、重试、停止、审批和外部副作用组合场景下最终收敛。

同时必须建立**升级体检**：在锁定版本上真实加载插件并 typecheck，确保 DSH 改字段名时在升级时立刻报错，而不是运行时静默失效（见 [技术文档](../02-实现/01-DSH契约面与AgentCore能力映射.md) §3）。

核查未通过前，允许产品裁定、插件原型和受控实验；禁止冻结完整数据库、API、完整状态机、权限模型和正式排期。

## 7. 首版验收

- 代理调用临时 Agent不会产生长期成员。
- CEO 单次协作完成后可回到代理，目标、产物和责任连续。
- CEO 派发任务后，成员失败或部分完成不会被包装成成功，且交付必须带落盘物证。
...
|---|---|---|
| P0 | 产品基线与对象边界 | 完成 |
| P1 | DSH 插件契约收窄与升级体检 | 进行中 |
| P2 | 状态持久化 + 交付契约与验收账本 | 待开始 |
| P3 | 记忆与统一检索 | 待开始 |
| P4 | 编排调度增强与后续能力域 | 后续 |

> 阶段定义与 [PRD-01](PRD-01-Magic产品总览.md) §24 保持一致。工程组织已于 2026-09-15 移除，不在其中。
...
成员失败与阻塞的呈现不得只以颜色或图标示意：失败成员在成员工作区渲染结构化失败卡，给出失败工具、错误摘要与未完成内容，并提供「重试」与「交回 CEO 重规划」两个动作（经既有重规划通道执行）；阻塞成员的阻塞卡指明等待的上游成员；画布上的阻塞成员节点带阻塞徽标并提示等待对象。

> **2026-09-13 用户裁定**：任务板是**只读视图**。任务生命周期完全由智能体驱动——CEO 在主会话沟通后拆解派发（createTask），成员完成工作后带 CAS 与证据回写（updateTask）；前端不提供人工新建或完成任务的入口，人的角色是沟通与拍板，不是手工维护任务清单。成员已完成而任务仍待处理这类状态漂移按「状态诚实」原则原样透出，交由 CEO 处理，不得用人工点击抹平。任务板的唯一展示面是画布（任务泳道 + 任务节点），右坞成员工作区不重复呈现任务板；右坞 tab 以当前选中成员命名，不再使用泛化的「成员工作区」标题。

## 13. 底座契约组合验收（原 F-10）

锁定 DSH 版本、Profile/Bundle、插件加载方式和扩展点清单后，必须先在同一证据包中确定 Magic `TaskAttempt` 对应哪些可观察底座证据，再验证：断连、重启、重试、停止、审批等待、权限变化、文件/Git/MCP/网络副作用、责任收口和产物来源。任一组合场景不能解释清楚，首次正式开发保持 No-Go。

【已确认产品规则】Magic 任务账本是真相源、对象分层、责任唯一、状态诚实、历史不可静默改写。

【首版建议】单工作区最小任务闭环和 S-01 至 S-12 场景。

【待用户裁定】未来多用户任务协作和复杂恢复策略。
...
> **【V1 范围注记】** V1 的运行/成员状态以 Magic 插件当前实现为准：`plugins/magic-ceo/src/plan.ts:1-10` 的 `RunPhase`（`queued / running / completed / failed / skipped / cancelled / unverified / unknown_after_restart / blocked`）。本图的"等待审批、暂停"在 V1 中延后，审批等待以"审批卡 + 横幅"呈现，不改变状态机。
...
## 21. 底座契约 Go/No-Go（原 F-10）

这不是一份说明文档，而是一组必须可重复运行的证据：固定 DSH 版本、Profile/Bundle、插件加载方式和扩展点清单，并跑通升级体检（真实加载 + typecheck）；先确定 Attempt 与底座执行证据的最小绑定模型，再执行断连、重启、重试、停止、审批、权限变化、文件/Git/MCP/网络副作用等组合场景；最终证明 Task、Attempt、底座执行证据、Approval、Artifact、Responsibility、SideEffect 和费用状态可以诚实收敛。任一项只能靠猜测时，首次正式开发保持 No-Go。
