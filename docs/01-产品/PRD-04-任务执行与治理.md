...
> **本版要点**：删除已作废的 domain 源码引用，运行状态改以插件实现为准（§3、§16）；F-10 改名并重定义为"底座契约核查"（§6、§13、§21）；阶段表对齐 PRD-01（§8）；工程相关对象标注暂缓（§9）。
...
> 阶段定义与 [PRD-01](PRD-01-Magic产品总览.md) §24 保持一致。工程组织不在其中，见 [PRD-03](PRD-03-工程组织与协作.md)（暂缓）。
...
| ProjectMember / PM | Agent 在工程中的长期关系/协调关系（暂缓） | 用户；停用/交接/归档 | Magic 工程组织 |
