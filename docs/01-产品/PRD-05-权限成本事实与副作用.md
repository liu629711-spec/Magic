...
> **本版要点**：授权范围从"工程"统一改为"工作区"（工程暂缓，见 [PRD-03](PRD-03-工程组织与协作.md)）；角色表述对齐 CEO 工作方式。
...
一个人是 CEO、成员或临时 Agent，不代表他天然拥有全部权限。每次动作都根据以下 9 个字段判断：

| 字段 | 白话解释 |
|---|---|
| 主体 | 谁要做：用户、CEO、成员或临时 Agent |
| 资源 | 对什么做：文件、仓库、外部系统或项目事实 |
...
2. **当前范围内自动处理**：在当前工作区、资源、预算和有效期内自动执行，超出范围重新询问。
3. **完全访问当前工作区**：允许工作区范围内常规动作自动执行，但仍受系统禁止项、高风险规则、预算上限和环境限制约束。

三档只是简化入口，不替代内部 allow / ask / deny，也不能越过上级拒绝或系统硬拒绝。

## 4. 授权继承与向下派发

上级 Agent 派发任务时，只能传递自己被允许转授权的最小范围。派发必须限制资源、动作、期限、预算和递归深度；不能把原始凭据、全部私有记忆或整个工作区权限直接传给下级。

权限变化只影响后续调用。撤销或过期不会把已经发生的运行、写入、费用和外部副作用改写掉；系统必须在历史记录中显示“授权何时生效、何时撤销、期间发生了什么”。

## 5. 成本治理

每个任务至少记录四个数字：预算上限、已预留、实际消耗、剩余额度。成本要能归属到任务、TaskAttempt、Agent 和外部服务。
...
2. 查找当前任务、会话、工作区和账户层授权，按作用域和有效期筛选。
...
预算上限由用户或任务策略给出，预留发生在派发或高成本动作前，实际消耗来自底层计量，剩余为上限减去预留和实际消耗。预留失败时不能静默派发；实际消耗超过预估时，系统暂停后续高成本动作并请求决定。
...
任务需要什么就提供什么：目标和约束、必要文件/产物、相关结构化结果、已确认项目事实。默认不提供兄弟任务 transcript、完整会话、私有 memory、NoteWall、凭据和跨工作区内容。Agent 请求超范围信息时，必须说明目的、资源、期限、影响和授权来源；批准后也只授予最小范围。
...
会话是 Agent 展示执行过程的第一工作现场。权限、成本、事实和副作用信息应在相关动作发生时进入会话上下文，并可通过当前对象检查视图展开查看；不要求把所有治理对象长期堆在一个万能 Tab 中。团队信息和审计详情按需打开，具体位置由前端设计决定。
...
| 给定条件 | 用户动作/系统事件 | 期望结果 |
|---|---|---|
| 工作区内低风险读文件 | Agent 发起读取 | 自动允许，记录资源和结果 |
| 需要写入当前工作区文件 | Agent 发起写入 | 按三档授权处理，显示范围并记录 ChangeSet |
| 预算接近上限 | 新 Attempt 需高成本调用 | 暂停或询问，不静默超支 |
| 用户撤销权限 | 旧 Attempt 已写文件 | 后续调用被拒绝，旧写入和费用仍可查 |
| 成员请求跨工作区文件 | 无跨区授权 | 默认拒绝并解释边界 |
...
【首版建议】单用户单工作区、当前工作区内资源、低风险自动和高风险逐次确认。

【待用户裁定】多用户共享授权、跨工程借调（未来工程）、自动补偿和长期预算策略。
