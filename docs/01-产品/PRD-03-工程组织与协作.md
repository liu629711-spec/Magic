...
| 用户直达成员 | 用户明确要求成员承担独立交付 | 必须进入工程任务账本；普通对话仍留在会话中 |
...
用户直接联系项目成员并明确要求其承担独立交付时，系统创建工程任务并默认让该成员承担交付责任，同时通知 PM。普通咨询、补充或继续对话只追加会话回合，不自动进入工程任务账本。若新任务影响工程目标、优先级、资源、预算或交付，PM必须给出“接受、调整计划、暂停冲突部分、请求用户选择”之一；通知不等于所有工作都必须等待 PM 批准。
...
工程信息不要求全部塞进一个固定 Tab。会话负责当前 Agent 的工作过程；工程总览、PM/成员列表、任务账本、资源状态、交接卡和冲突合并卡作为按需打开的工程视图或当前对象检查视图。后端必须保证同一工程唯一 PM、任务责任归属、成员生命周期、资源锁定、ChangeSet/MergeTask 关系和事实版本；首版验收覆盖建工程、PM 交接、PM 失联待接管、用户直达成员、文件失联、并发修改和归档重开。
...
- 不影响原目标的补充：先追加会话回合；只有需要独立交付时才创建新任务并并行执行。
