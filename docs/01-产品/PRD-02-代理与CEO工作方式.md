...
> 2026-09-13 新增。Agent 不只会在本机读写文件，还能自己开一个真浏览器去干活。

### 18.1 一句话说清

Agent 可以自己开一个真 Chromium：查资料、翻页、填表、点击、下载文件、用已登录的网站。你像看直播一样实时看着它操作，随时亲手接管，接管完交还给它继续。

### 18.2 它怎么运转

```mermaid
flowchart LR
    A[你说：去查某题并下载资料] --> B[Agent 调用浏览器工具]
    B --> C[真 Chromium 打开页面干活]
    C --> D{需要人吗？}
    D -- 不需要 --> E[继续干到交付]
    D -- 验证码 / 登录 / 对外提交 --> F[观察窗弹提示，停下等你]
    F --> G[你亲手点掉 / 输入] --> E
    C -. 实时直播 .-> H[观察窗：侧边栏「Agent 浏览器」tab]
    G -. 同一路直播 .-> H
```
...
| 谁干 | 干什么 |
|---|---|
| Agent | 搜索、翻页、点击、填表、下载、截图取证——都是可重来的动作 |
| 人 | 验证码、输密码、以及一切「对外提交」类动作（下单、发帖、发消息——不可逆，按 [PRD-05](PRD-05-权限成本事实与副作用.md) §7 分级走审批） |

### 18.4 边界

- 浏览器是 Agent 专用的一只 Chromium（登录态只存本机），不是你日常在用的那个浏览器。
- 下载的文件落在会话工作区，和其它产物同一套管理规则。
- 「对外提交」永远等人点头，Agent 不能替你按。
- 无图形界面的会话只有工具、没有直播窗。
...
| 前置条件 | 用户动作/系统事件 | 预期结果 |
|---|---|---|
| 代理会话 | 让 Agent 去某页收集资料并下载 | 观察窗实时显示它的操作；文件落工作区 |
| 目标网站弹验证码 | — | 观察窗提示并等待；人接管点掉后 Agent 继续 |
| 任务要求「提交订单」 | Agent 进行到最后一步 | 停下等审批，不自动提交 |
| Agent 用的浏览器 | 用户在观察窗里直接点击 | 点击作用于同一个浏览器；Agent 感知页面变化并继续 |
