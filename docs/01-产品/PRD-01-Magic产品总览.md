...
Magic 是建立在 OpenCode 之上的统一 AI 工作台。OpenCode 提供 Agent 执行、工具、文件、终端、底层会话和运行能力；Magic 在其上建立用户可理解的工作方式、长期工程组织、任务账本、责任体系、信息共享、权限治理和真实交付。
...
- 底层 Session、Turn、Run、事件和运行证据；
- 底层协议、服务和客户端能力。

OpenCode 的 Session、Run、Agent、worker、事件和工具 approval 不能直接替代 Magic 的任务、工程、PM、责任、事实、产物和组织授权。具体绑定关系必须以锁定版本的源码、实验和实机证据为准。

```mermaid
flowchart LR
  U[用户目标] --> M[Magic 产品层]
  M -->|任务 责任 权限 事实 交付| O[OpenCode 执行底座]
...
工作过程中，用户随时能回答五个问题：现在在做什么、谁负责、系统在等什么、已经产生了什么结果、下一步需要我决定什么。用户不需要观看每个工具调用，但不能看不到失败、权限请求、资源冲突、预算超限或已经发生的副作用。

主工作台至少显示：当前目标、当前责任人、任务真实状态、当前计划步骤、下一步、正在协作的成员、结果和产物、已经发生的动作、需要用户批准的事项、失败和阻塞原因。底层 Session、Run、工具日志和完整 transcript 可以折叠，但不能用折叠隐藏失败、同步缺口、预算超限、权限拒绝或不可逆副作用。
...
Magic 对象负责产品意义和最终收口；OpenCode Session、Turn、Run、事件和工具结果只负责底层执行证据。任何“已完成”都必须由 Magic 任务责任人基于结果和验收确认。
...
- **待技术核查**：Magic 对象与 OpenCode Session/Run 的绑定、事件重放、断连恢复和底座权限能力。
...
OpenCode 或 AgentCore 中存在代码、测试或历史做法，只能说明“可能继承”或“需要适配”，不能直接写成 Magic 已支持。产品 PRD 的日期、版本、负责人和证据链接必须随结论一起维护。
...
AgentCore 的历史实践可以提供协作、状态和恢复方面的经验，但它不是 Magic 首版必须经过的生产运行层。首版优先验证 OpenCode 执行底座与 Magic 产品层的边界；是否把 AgentCore 某项能力纳入生产链路，必须同时满足技术证据和实际收益，不能因为旧项目有过实现就自动继承。
...
| P1 | OpenCode 底座与运行事实 | 版本、runtime、协议可复现 |
