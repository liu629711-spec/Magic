...
OpenCode 的 Session、候选执行对象、Agent、worker、事件和工具 approval 不能直接替代 Magic 的任务、工程、PM、责任、事实、产物和组织授权。当前不得预设存在可一对一绑定的 OpenCode `Run`；具体绑定关系必须以锁定版本的源码、实验和实机证据为准。
...
Magic 对象负责产品意义和最终收口；OpenCode 的 Session、输入/响应、事件、实际执行对象和工具结果只负责底层执行证据。具体对象是否可被一个 Magic Attempt 关联、以及关联基数，均待技术核查。任何“已完成”都必须由 Magic 任务责任人基于结果和验收确认。
