...
12. **会话状态生命周期（2026-09-16，依据用户提供的会话状态流转规范 code.html）**：`running` 白笔运笔动效；`interrupted`（中断/异常停止）笔停下变红（静态 #ef4444，无运笔动画）；`completed` 绿色勾（「墨凝收笔与翡翠微绽」方案一：`bg-tertiary/20` 圆 + check，`emeraldBloom` 420ms 绽放一次后静止）；**点击完成/中断的会话行 → 状态恢复 idle（回圆点）**。不使用文字角标。
