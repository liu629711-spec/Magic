...
15. **对话区采用 codex-ui（2026-09-17）**：中栏对话流**不抄 DSH web ui-chat**（其折叠层虽可剥离但 codex-ui 更优），改用用户确认可用的现成组件库 `reference-project/codex-ui`（纯 React 零依赖，props 进 `ConversationMessage[]` 出 `onSendMessage/onApprove`）——整库搬入 `apps/magic-desktop/src/vendor/codex-ui/`，165 处颜色 token 化为 `--cv-*` 变量（暗色映射 stitch token，`.vendor-cv-light` 保留浅色原值）；stitch 设计稿降为参考图，「浮夸内容不要」（2026-09-17 用户裁定）：按钮全中性（验证卡/发送键）、运行指示低调呼吸、执行计划进度条纯灰阶。SDK 接线时写 `session.event → ConversationMessage` 转换层（可参考 `dsh-codex-ui` 插件的同族转换逻辑）。
